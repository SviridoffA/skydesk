...
//  $query="select dv_log.uid,max(start) as last  from dv_log where start > '2015-01-01'  group by uid order by last desc";
  $query="select users.*,users_pi.*,users_last.*,bills.* from users,users_pi,users_last,bills where bills.uid=users.uid and bills.deposit >=0 and users.uid=users_pi.uid and users.uid=users_last.uid and users.disable=0 order by users_last.last desc";
