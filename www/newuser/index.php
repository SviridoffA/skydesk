...
function get_street($address) {
  ...
  $query="SELECT * FROM streets WHERE id=$address";
...
for ($i=0;$i<$num;$i++) {
  ...
  $address=mysql_result($res,$i,"users_pi.address_street");
  if(preg_match('/\d+/', $address)){
    $address=get_street($address);
    $address="<br>".$address;
    }
  else{
    $address="<br>".$address;
  ...
  echo "<tr><td>$k</td><td><a href=https://91.223.48.25:9443/admin/index.cgi?index=15&UID=$uid target=_blank>$uid</a></td><td>$fio</td><td>$address</td><td>$registration</td><td>$metr</td><td>$metr_digit</td><td>$vols</td><td>$vols_digit</td><td>$descr</td><td>$total</td></tr>";
