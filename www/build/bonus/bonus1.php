...
function check_correct($uid,$sdate) {
  mysql_connect("91.223.48.25","root","");
  mysql_set_charset("utf8");
  $query="select sum(sum) as total from payments where method=5 and uid=$uid and date = '2015-03-01 00:00:00'"; 
  $res=mysql("abills",$query);
  echo mysql_error();
  $payments=mysql_result($res,0,"total");
  return($payments);  
...
$query="select * from payments where method=4 and date >= '2014-06-04' and date < '2015-03-01' order  by date";
$res=mysql("abills",$query);
$num=mysql_num_rows($res);
echo "<table border=1>";
$k=0;
