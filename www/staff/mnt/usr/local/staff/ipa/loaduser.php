...
$days=7;
$query="select year(subdate(now(), interval $days day)) as y,month(subdate(now(),interval $days  day)) as m,dayofmonth(subdate(now(), interval $days day)) as d from customers and username not like 'med'";
$res=mysql("mvs",$query);
echo mysql_error();
