...
// $cmd="/usr/local/bin/snmpwalk -v 1 -c public -On 195.72.157.250 1.3.6.1.2.1.17.7.1.2.2.1.2";
 $cmd="/usr/local/bin/snmpwalk -v 1 -c tenretni -O n 10.77.78.3 1.3.6.1.2.1.4.22.1.2";
// echo $cmd;
$pp=popen($cmd,"r");
while (!feof($pp)) {
...
//  echo $str."\n";
  $str=ereg_replace(".1.3.6.1.2.1.4.22.1.2.","",$str);
//  echo $str."\n";
//  preg_match("/.1.3.6.1.2.1.17.7.1.2.2.1.2.([\d]+).([\d]+).([\d]+).([\d]+).([\d]+).([\d]+).([\d]+) = INTEGER: ([\d]+)/",$str,$mem,PREG_OFFSET_CAPTURE,0);
  preg_match("/([\d]+).([\d]+).([\d]+).([\d]+).([\d]+) = STRING: ([\d\w]+):([\d\w]+):([\d\w]+):([\d\w]+):([\d\w]+):([\d\w]+)/",$str,$mem,PREG_OFFSET_CAPTURE,0);
//  preg_match("/([\d]+).([\d]+).([\d]+).([\d]+) = STRING: ([\d\w]+) ([\d\w]+) ([\d\w]+) ([\d\w]+) ([\d\w]+) ([\d\w]+)/",$str,$mem,PREG_OFFSET_CAPTURE,0);
//  var_dump($mem);
  $ips=$mem[2][0].".".$mem[3][0].".".$mem[4][0].".".$mem[5][0];
  $mac1=sprintf("%s",strtolower($mem[6][0]));
  if (strlen($mac1) == 1) { 
    $s1="0".$mac1;
    $mac1=$s1;
  }
  $mac2=sprintf("%+02s",strtolower($mem[7][0]));
  if (strlen($mac2) == 1) { 
    $s1="0".$mac2;
    $mac2=$s1;
  }
  $mac3=sprintf("%+02s",strtolower($mem[8][0]));
  if (strlen($mac3) == 1) { 
    $s1="0".$mac3;
    $mac3=$s1;
  }
  $mac4=sprintf("%+02s",strtolower($mem[9][0]));
  if (strlen($mac4) == 1) { 
    $s1="0".$mac4;
    $mac4=$s1;
  }
  $mac5=sprintf("%+02s",strtolower($mem[10][0]));
  if (strlen($mac5) == 1) { 
    $s1="0".$mac5;
    $mac5=$s1;
  }
  $mac6=sprintf("%+02s",strtolower($mem[11][0]));
  if (strlen($mac6) == 1) { 
    $s1="0".$mac6;
    $mac6=$s1;
  }
...
//  $mac=dechex($mem[2][0]).":".dechex($mem[3][0]).":".dechex($mem[4][0]).":".dechex($mem[5][0]).":".dechex($mem[6][0]).":".dechex($mem[7][0]);
  $port=$mem[1][0]-3000;
  $query="insert into ipmac(ip,mac,cdate,fdate,switch,port) VALUES('$ips','$mac',now(),now(),'195.72.157.251','$port')";
  $res=mysql("mvs",$query);
  $err=mysql_error();
  echo $query;
  echo $err;
  if (!empty($err)) {
    $query="update ipmac set cdate=now(),port='$port' where mac like '$mac' and ip like '$ips' and port like '$port' and switch like '195.72.157.251'";
    $res=mysql("mvs",$query);
    echo "$query";
  } else {
//    echo "not update";  
