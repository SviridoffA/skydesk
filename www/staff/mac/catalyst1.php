...
//  $mac=dechex($mem[2][0]).":".dechex($mem[3][0]).":".dechex($mem[4][0]).":".dechex($mem[5][0]).":".dechex($mem[6][0]).":".dechex($mem[7][0]);
  $port=$mem[1][0];
  $query="insert into ipmac(ip,mac,cdate,fdate,switch,port) VALUES('$ips','$mac',now(),now(),'91.223.48.249','$port')";
  $res=mysql("sky_switch",$query);
  $err=mysql_error();
//  echo "$query\n";
//  echo $err;
  if (!empty($err)) {
    $query="update ipmac set cdate=now() where mac like '$mac' and ip like '$ips' and switch like '91.223.48.249' and port like '$port'";
    $res=mysql("sky_switch",$query);
//    echo "update";
  } else {
//    echo "not update";  
